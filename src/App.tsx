...
import {
  ...
  ChevronRight, Calendar, BarChart3, Settings, Sparkles, 
  Trash2, ArrowRight, Play, Check, ShieldCheck, Sun, Moon,
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'tasks' | 'timeline' | 'stats'>('dashboard');
  ...
  const plannedBedTimeMinutes = 23 * 60; // 11:00 PM

  // Dynamic schedule health check
  ...
  const currentFocus = useMemo(() => {
    ...
    // Active task currently in window or closest next task
  ...
  // ⚡ FIX MY DAY ALGORITHM
  const handleFixMyDay = () => {
    let currentPointer = Math.max(currentMinutesOfDay + 10, timeToMinutes('09:00')); // Start 10 mins from now
    ...
    const reorganized = tasks.map(t => {
      ...
      if (matchedUncompleted) {
        ...
        currentPointer += t.duration + 15; // Add 15 min buffer
  ...
  return (
    <div className="min-h-screen bg-[#090D16] text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column (Primary Focus & Tasks) */}
        <section className="lg:col-span-8 space-y-6">
          ...
          {/* Smart Warning Banner (If Slipping) */}
          {scheduleStats.health !== 'ON_TRACK' && (
        ...
        {/* Right Column (Timeline & Smart Reorganizer Side Panel) */}
        <section className="lg:col-span-4 space-y-6">
          
          {/* Action Box: Fix My Day */}
