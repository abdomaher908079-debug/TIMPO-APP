...
export default function App() {
  ...
  // Persistence via localStorage - Starting completely empty as requested
  ...
  // Streak logic with last active date check
  const [streak, setStreak] = useState<number>(() => {
    const savedStreak = localStorage.getItem('tempo_streak');
    const lastActive = localStorage.getItem('tempo_last_active');
    ...
    if (savedStreak && lastActive) {
      ...
      if (diffDays > 1) {
        ...
        localStorage.setItem('tempo_streak', '0');
        return 0;
      }
      return Number(savedStreak);
    }
    return 0;
  });
  ...
  // Gallery image picker helper (converts local file to Base64 to store in localStorage)
  ...
  const toggleTask = (id: string) => {
    setTasks(tasks.map(task => {
      if (task.id === id) {
        ...
        localStorage.setItem('tempo_last_active', todayStr);
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
      ...
      {/* Profile Modal with Gallery Upload */}
      {isProfileModalOpen && (
          ...
          <div className="relative bg-slate-900 border border-indigo-500/40 w-full max-w-md rounded-3xl p-6 shadow-2xl z-10 space-y-5 animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
              <div className="flex flex-col items-center gap-3">
                ...
                {/* Gallery Button for Profile Avatar */}
                <label className="cursor-pointer bg-indigo-600/20 hover:bg-indigo-600/30 border border-indigo-500/40 text-indigo-300 text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-2 transition-all">
      ...
      <main className="flex-1 max-w-xl w-full mx-auto p-4 pb-24 space-y-6">
        ...
        {activeTab === 'social' && (
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            {/* Create Post Form with Gallery Image Picker */}
            <form onSubmit={addPost} className="bg-slate-950/90 border border-indigo-500/30 rounded-3xl p-4 shadow-xl space-y-3">
              ...
              {newPostImage && (
                <div className="relative rounded-xl overflow-hidden max-h-40 border border-indigo-500/30">
                  <img src={newPostImage} alt="Upload preview" className="w-full h-full object-cover" />
                  <button 
                    type="button" 
                    onClick={() => setNewPostImage(undefined)}
                    className="absolute top-2 right-2 bg-black/70 p-1.5 rounded-full text-white hover:bg-rose-600 transition-colors"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <div className="flex items-center justify-between pt-1">
                <label className="cursor-pointer bg-slate-900 hover:bg-slate-800 border border-slate-800 text-indigo-300 px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all">
                  ...
                  <input 
                    ...
                    accept="image/*" 
                    className="hidden" 
                    onChange={(e) => handleImageUpload(e, (url) => setNewPostImage(url))}
                  />
                </label>
